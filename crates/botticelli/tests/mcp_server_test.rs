...
fn test_mcp_server_startup() {
    ...
    assert!(health_response.status().is_success(), 
        "Health endpoint returned non-success status: {}", health_response.status());
    
    let health_json: serde_json::Value = health_response.json()
        .expect("Failed to parse health response as JSON");
    
    println!("Health response: {}", serde_json::to_string_pretty(&health_json).unwrap());
    assert_eq!(health_json["status"], "healthy", "Expected status to be 'healthy'");
    ...
    assert!(info_response.status().is_success(), 
        "Info endpoint returned non-success status: {}", info_response.status());
    
    let info_json: serde_json::Value = info_response.json()
        .expect("Failed to parse info response as JSON");
    
    println!("Server info: {}", serde_json::to_string_pretty(&info_json).unwrap());
    
    // Verify version field exists and is non-empty
    let version = info_json["version"].as_str()
    ...
    assert!(version.split('.').count() >= 2, 
        "Server version should follow semver format: {}", version);
    
    // Verify server has a name
    let name = info_json["name"].as_str()
        ...
        Ok(_) => {
            ...
            match tools_result {
                Ok(tools) => {
                    ...
                    assert!(!tools.is_empty(), "Expected at least some tools to be available");
