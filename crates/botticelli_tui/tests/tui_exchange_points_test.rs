...
async fn test_exchange_point_event_capture() {
    ...
    assert!(
        send_elapsed.as_micros() < 100,
        "Channel send took {:?} (> 100µs)",
        send_elapsed
    );
...
async fn test_exchange_point_event_routing() {
    ...
    tracing::info!(
        "Event routing → AppState.handle_key(): {:?}",
        handle_elapsed
    );
    assert!(
        handle_elapsed.as_millis() < 5,
        "Event handling took {:?} (> 5ms)",
        handle_elapsed
    );
}
...
async fn test_exchange_point_state_update() {
    ...
    assert!(
        update_elapsed.as_micros() < 500,
        "State update took {:?} (> 500µs)",
        update_elapsed
    );
}
...
async fn test_exchange_point_render_pipeline() {
    ...
    assert!(
        send_total.as_micros() < 1000,
        "Sending 11 events took {:?} (> 1ms)",
        send_total
    );
...
async fn test_exchange_point_full_pipeline() {
    ...
    assert!(
        total_latency.as_millis() < 1,
        "Full pipeline took {:?} (> 1ms)",
        total_latency
    );
}
