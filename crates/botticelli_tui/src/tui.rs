...
impl Tui {
    ...
    pub fn with_llm(
        chat_host: Option<Arc<tokio::sync::Mutex<dyn botticelli_interface::ChatHost>>>,
    ) -> TuiResult<Self> {
        ...
        self.terminal.draw(|frame| {
            ...
            let result = match state.mode() {
                ...
                ViewMode::NarrativeBrowser => {
                    crate::view::NarrativeBrowserView.render(frame, state)
                }
                ViewMode::ConversationHistory => {
                    crate::view::ConversationHistoryView.render(frame, state)
                }
