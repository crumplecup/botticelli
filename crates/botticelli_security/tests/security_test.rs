...
fn test_approval_create_action() -> anyhow::Result<()> {
    ...
    let action_id = workflow.create_pending_action("narrative1", "test.command", params, None)?;
...
fn test_approval_approve_action() -> anyhow::Result<()> {
    ...
    workflow.approve_action(&action_id, "admin", Some("Looks good".to_string()))?;
...
fn test_approval_deny_action() -> anyhow::Result<()> {
    ...
    workflow.deny_action(&action_id, "admin", Some("Not allowed".to_string()))?;
