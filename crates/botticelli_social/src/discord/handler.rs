...
impl BotticelliHandler {
    ...
    async fn store_guild<'a>(&'a self, guild: &'a Guild) -> DiscordResult<()> {
    ...
    async fn store_member<'a>(&'a self, guild_id: GuildId, member: &'a Member) -> DiscordResult<()> {
    ...
    async fn store_role<'a>(&'a self, guild_id: GuildId, role: &'a Role) -> DiscordResult<()> {
