...
use crate::{CarouselConfig, Narrative};
...
#[derive(Debug, Clone, Builder, Serialize, Deserialize, JsonSchema, Elicit, Getters, Default)]
...
pub struct PartialNarrative {
    ...
    /// Act execution order.
    pub act_order: Vec<String>,

    /// Act definitions (name -> PartialAct).
    pub acts: HashMap<String, PartialAct>,
...
impl PartialNarrative {
    ...
    #[tracing::instrument(skip(self))]
    pub fn validate(&self) -> McpResult<crate::validator::ValidationResult> {
        let toml = self.to_toml()?;
        Ok(crate::validator::validate_narrative_toml(&toml))
    }
    ...
    #[tracing::instrument(skip(self))]
    pub fn try_into_narrative(&self) -> McpResult<Narrative> {
        let toml = self.to_toml()?;
        ...
        Narrative::from_toml_str(&toml, self.name.as_deref())
