...
pub trait NarrativeElicitor: Send + Sync {
    /// Error type for elicitation operations.
    type Error: std::error::Error + Send + Sync + 'static;

    /// Partial narrative type being constructed.
    ...
    fn can_run(&self, partial: &Self::PartialNarrative) -> bool;
    ...
    async fn elicit(
        &self,
        dialog: &mut dyn ElicitationDialog<Error = Self::Error>,
        partial: &mut Self::PartialNarrative,
    ) -> Result<(), Self::Error>;

    /// Check if this aspect is complete.
    ///
    /// Returns true if no more information is needed.
    fn is_complete(&self, partial: &Self::PartialNarrative) -> bool;

    /// Suggest what to do next (optional guidance).
    ///
    /// Returns None if no specific suggestion.
    fn suggest_next(&self, partial: &Self::PartialNarrative) -> Option<String> {
        let _ = partial;
        None
    }
}
