...
async fn test_core_tools_registered() {
    ...
    assert!(tool_names.iter().any(|t| t.name == "echo"), "echo tool should be registered");
    assert!(tool_names.iter().any(|t| t.name == "server_info"), "server_info tool should be registered");
