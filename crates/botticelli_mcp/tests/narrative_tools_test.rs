...
fn test_create_narrative_result_serialization() {
    ...
    assert!(json["toml_with_comments"].as_str().unwrap().contains("# Generated"));
...
async fn test_create_narrative_basic() {
    ...
    assert!(result.act_count >= 2, "Expected at least 2 acts, got {}", result.act_count);
}
...
async fn test_create_narrative_complex_description() {
    ...
    let params = CreateNarrativeParams {
        description: "First fetch user data, then process the data, and finally generate a summary report".to_string(),
    ...
    assert!(result.0.act_count >= 2, "Should have extracted multiple acts");
...
fn test_modify_narrative_params_serialization() {
    ...
    assert!(json["narrative_toml"].as_str().unwrap().contains("[narrative]"));
...
";
    ...
    assert!(result.changes.iter().any(|c| c.contains("Removed act 'process'")));
}
...
";
    ...
    assert!(result.toml.contains("model = \"claude-3-5-sonnet-20241022\""));
...
";
    ...
    assert!(result.changes.iter().any(|c| c.contains("Changed temperature")));
}
...
fn test_save_narrative_result_serialization() {
    let result = SaveNarrativeResult::new(
...
async fn test_save_narrative_creates_directories() {
    ...
    assert!(file_path.parent().unwrap().exists(), "Parent directories should exist");
    assert!(file_path.exists(), "File should exist");
}
