...
async fn test_echo_tool() {
    ...
    assert!(
        response.get("echo").is_some(),
        "Response should have 'echo' field"
    );
    assert_eq!(
...
async fn test_server_info_tool() {
    ...
    assert!(
        result.is_ok(),
        "ServerInfo tool should succeed: {:?}",
        result.err()
    );
...
fn test_expected_tool_count() {
    ...
    println!(
        "Expected tool count with current features: {}",
        expected_count
    );
    assert!(expected_count >= 6, "Should have at least 6 base tools");
    assert!(
        expected_count <= 26,
        "Should have at most 26 tools with all features"
    );
}
