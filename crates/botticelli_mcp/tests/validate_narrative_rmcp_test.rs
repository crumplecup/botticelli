...
"#;
    ...
    let result = server.validate_narrative(Parameters(params))
        .await
...
async fn test_validate_narrative_neither_content_nor_file() {
    ...
    assert!(result.is_err(), "Should fail when neither content nor file_path provided");
}
...
"#;
    ...
    if !result.0.warnings.is_empty() {
        assert!(!result.0.valid, "Should be invalid in strict mode with warnings");
    }
}
...
"#;
    ...
    assert!(result.0.summary.contains("error"), "Summary should mention errors");
    assert!(result.0.summary.contains("warning"), "Summary should mention warnings");
}
...
async fn test_validate_narrative_result_serialization() {
    ...
    let result = ValidateNarrativeResult::new(
        true,
        vec![],
        vec![],
    );
