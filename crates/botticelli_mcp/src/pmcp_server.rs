...
/// Runs the PMCP-based MCP server on HTTP transport.
#[instrument(skip(db_ops))]
pub async fn run_pmcp_http_server(
    ...
    info!("Starting PMCP-based MCP server on HTTP");
    ...
    let addr_str = format!("http://{}:{}", host, port);
    tracing::info!("Server built successfully, running on HTTP at {}", addr_str);

    // Create HTTP transport config
    let base_url = addr_str.parse().map_err(|e| {
        let kind = botticelli_error::McpErrorKind::InvalidInput(format!("Invalid URL: {}", e));
        botticelli_error::McpError::new(kind)
    })?;
    
    let config = pmcp::HttpConfig {
        base_url,
    ...
    let transport = pmcp::HttpTransport::new(config);
