...
/// Tool for boolean confirmation (yes/no).
///
/// This tool corresponds to the `elicit_bool` MCP tool expected by
/// the elicitation crate for Affirm paradigm types (bool).
pub struct ElicitBoolTool {
    dialog: Arc<DialogResource>,
}

impl ElicitBoolTool {
    /// Create a new boolean elicitation tool.
    pub fn new(dialog: Arc<DialogResource>) -> Self {
        Self { dialog }
    }
}
