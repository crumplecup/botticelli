...
pub async fn elicit_acts_refactored(
    ...
    let (act_order, acts) = match approach {
        ActApproach::AutoExtract => extract_from_description_refactored(partial).await?,
        ActApproach::ManualCount => count_based_specification_refactored(&client).await?,
        ActApproach::Interactive => one_by_one_specification_refactored(&client).await?,
    };
...
async fn extract_from_description_refactored(
...
async fn count_based_specification_refactored(
...
#[instrument(skip(client))]
async fn one_by_one_specification_refactored(
    client: &pmcp::Client<botticelli_mcp::InProcTransport>,
) -> BotticelliResult<(Vec<String>, HashMap<String, PartialAct>)> {
    let mut act_order = Vec::new();
    let mut acts = HashMap::new();
