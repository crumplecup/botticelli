...
async fn test_create_narrative_tool() -> ChatResult<()> {
    ...
    assert!(
        content.contains("narrative") || content.contains("created"),
...
async fn test_update_prompt_tool() -> ChatResult<()> {
    ...
    assert!(
        !response.as_text().contains("Error"),
...
async fn test_show_narrative_tool() -> ChatResult<()> {
    ...
    let create_cmd = Command::Narrative(NarrativeCommand::Create {
        prompt:
            "Create a narrative with one act that uses the echo tool to return 'Test successful'"
                .to_string(),
    });
    executor.execute(create_cmd).await?;
...
async fn test_validate_narrative() -> ChatResult<()> {
    ...
    // Should not error
    assert!(
        !response.as_text().contains("Error") || response.as_text().contains("valid"),
